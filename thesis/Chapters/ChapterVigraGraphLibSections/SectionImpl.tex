...
These concepts are explained in \cref{sec:graph_lib_python}.
...
\caption[VIGRA's graph classes]{ \label{tab:graph_classes}
    ...
    For node and edge id's we distinguish between dense ids
    which are continuous and sparse ids.
...
Edge descriptors are implemented as $N+1$-dimensional coordinate.
...
\caption[Grid Graph Ids]{ \label{fig:grid_graph_ids}
    ...
    To illustrate this, these ``virtual'' edges are showed dotted.
    ...
    showed in the same unique color.
...
an nodes while adding them to the graph . As a consequence, node ids can 
...
\subsection{Graph Algorithms} \label{sec:graph_graph_algorithms}
    ...
    \caption[Hierarchical Clustering Class Design]{ \label{fig:hc_class_design}
        ...
        These classes are responsible for feature merging, and selecting the edge
    ...
    with an \emph{cluster operator} implemented within the 
    \emph{ClusterOperatorConcept}.

    The cluster operator is connected to the merge
    graph via callbacks and is resposible for feature
    merging.
    Any edge contraction in the merge graph will call
    members of a cluster operator to merge node and edge features.
    From these merge features, the cluster operator needs
    to find the edge which needs to be contracted next.
    Often this is implemented with a 
    dynamic priority queue \footnote{
        A dynamic priority queue is usualy implemented as min 
        heap
    } where the edge with minimal weight can be accessed  and for any edge, the weight can be changed
    efficient.
    Then class design and interaction for hierarchical clustering is shown
    in \cref{fig:hc_class_design}.
